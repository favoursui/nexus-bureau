const ui = {
  ...
  toggleSidebar() {
    ...
    if (collapsed) {
      ...
      [logo, walletEl, gas, navLabel].forEach(el => el && el.classList.remove("hidden"));
    } else {
      sidebar.classList.add("collapsed");
      sidebar.style.width    = "56px";
      sidebar.style.minWidth = "56px";
      labels.forEach(l => l.classList.add("hidden"));
      [logo, walletEl, gas, navLabel].forEach(el => el && el.classList.add("hidden"));
    }
  },
  ...
  showWalletConnected(publicKey, balance) {
    const short = wallet.formatKey(publicKey);
    const bal   = this.formatBalance(balance);
    ...
    set("wallet-balance", `${bal} XLM`);

    // Wallet page
    const pageDisconnected = document.getElementById("wallet-page-disconnected");
    const pageConnected    = document.getElementById("wallet-page-connected");
    if (pageDisconnected) pageDisconnected.classList.add("hidden");
    if (pageConnected)    pageConnected.classList.remove("hidden");

    set("wallet-total",        bal);
  ...
  showWalletDisconnected() {
    ...
    const pageDisconnected = document.getElementById("wallet-page-disconnected");
    const pageConnected    = document.getElementById("wallet-page-connected");
    if (pageDisconnected) pageDisconnected.classList.remove("hidden");
    if (pageConnected)    pageConnected.classList.add("hidden");
  },
  ...
  updateWalletDisplay(info) {
    ...
    const bal   = usdc > 0 ? `${usdc.toFixed(3)} USDC` : `${xlm.toFixed(3)} XLM`;

    const set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
    set("wallet-short",        short);
    set("wallet-balance",      bal);
    set("wallet-total",        usdc > 0 ? usdc.toFixed(3) : xlm.toFixed(3));
    set("wallet-address-full", `${short} · Stellar Network`);
  },
    ...
    c.innerHTML = txs.map(tx => `
      <div class="flex items-center justify-between py-2.5 border-b border-subtle last:border-0">
        <div class="flex items-center gap-3 overflow-hidden min-w-0">
          <div class="w-8 h-8 min-w-8 rounded-full flex items-center justify-center"
        ...
        <div class="flex flex-col items-end ml-3 flex-shrink-0">
          <span class="text-sm font-mono" style="color:#f87171">
            -${parseFloat(tx.amount).toFixed(3)} ${tx.currency}
          </span>
          <a href="https://stellar.expert/explorer/testnet/tx/${tx.stellar_hash}"
             target="_blank"
             style="font-size:10px;color:var(--color-accent);text-decoration:none;">
            View tx ↗
          </a>
        </div>
      </div>
        ...
        <div class="log-payment">
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:10px;">
            <div style="display:flex;align-items:center;gap:8px;">
              ...
              <span class="text-sm font-semibold text-primary">Payment Confirmed</span>
            </div>
            <a href="${extra.explorer_url || `https://stellar.expert/explorer/testnet/tx/${extra.hash}`}"
               target="_blank"
               style="font-size:11px;color:var(--color-accent);text-decoration:none;white-space:nowrap;
                      padding:3px 8px;border:1px solid var(--color-accent);border-radius:6px;">
              View on Stellar Expert ↗
            </a>
          </div>

          <!-- Amount -->
          <p style="font-family:'JetBrains Mono',monospace;font-size:26px;font-weight:700;
                    color:var(--color-text-primary);margin-bottom:10px;">
            ${extra.amount}
            <span style="font-size:14px;color:var(--color-text-muted);font-weight:400;">${extra.currency}</span>
          </p>

          <!-- Transaction Hash -->
          <div style="padding:8px 10px;background:var(--color-bg-subtle);border-radius:8px;margin-bottom:6px;">
            <p style="font-size:10px;color:var(--color-text-muted);margin-bottom:3px;text-transform:uppercase;letter-spacing:0.05em;">
              Transaction Hash
            </p>
            <p style="font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--color-text-primary);
          ...
          <!-- Time -->
          <p class="text-xs text-muted font-mono">${time}</p>
      ...
      html = `
        <div class="log-entry">
          <div style="display:flex;align-items:flex-start;gap:10px;">
            <div style="width:8px;height:8px;min-width:8px;border-radius:50%;
                        background:${dotColor};margin-top:5px;flex-shrink:0;"></div>
            <p class="text-sm text-primary"
               style="word-break:break-word;overflow-wrap:break-word;line-height:1.5;">${message}</p>
          </div>
  ...
  addCommandMessage(text, isUser = false) {
    ...
    const html = isUser
      ? `<div class="msg-bubble-user">
           <p class="text-sm text-primary"
              style="word-break:break-word;overflow-wrap:break-word;">${cleaned}</p>
      ...
      : `<div class="msg-bubble-agent">
           ...
           <p class="text-sm text-primary"
              style="word-break:break-word;overflow-wrap:break-word;line-height:1.6;">${cleaned}</p>
           <p class="text-xs text-muted font-mono" style="margin-top:6px;">${time}</p>
         </div>`;
  ...
  updateGasTank(used, limit) {
    ...
    set("gas-percent", `${pct}%`);
    set("gas-used",    `${parseFloat(used).toFixed(3)} USDC used`);
  ...
  renderMarketplace() {
    const apis = [
      { name:"OpenWeather",   desc:"Real-time weather data",    price:"0.01", cat:"Weather",   icon:"🌤️" },
      { name:"Crypto Prices", desc:"Live crypto market data",   price:"0.01", cat:"Finance",   icon:"📈" },
      { name:"Crypto News",   desc:"Breaking crypto news",      price:"0.02", cat:"News",      icon:"⚡"  },
      { name:"Sentiment API", desc:"Market sentiment analysis", price:"0.05", cat:"AI",        icon:"🧠" },
    ...
    grid.innerHTML = apis.map(a => `
      <div class="api-card">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;">
          <div style="width:40px;height:40px;border-radius:10px;background:var(--color-bg-subtle);
                      border:1px solid var(--color-border);display:flex;align-items:center;
                      justify-content:center;font-size:18px;">${a.icon}</div>
          <span style="font-size:11px;padding:2px 8px;border-radius:20px;
                       background:var(--color-bg-subtle);color:var(--color-text-muted);">${a.cat}</span>
        </div>
        ...
        <p class="text-xs font-mono text-accent">${a.price} XLM / call</p>
      </div>
    `).join("");
  },
